...
#1. CNES - N. EAB + N. ESF 2020 ####
  CNES_ESF_Equipes <- readRDS(paste0(here("Previne","data","rds"),
                                   "/CNES_ESF_equipes_2020.RDS"))
    #Alternative CNES_ESF_unidades_2020_full.RDS
...
#2. CNES - % de UBS com horario de atendimento estendido####
  CNES_ESF_HorarioExt <- readRDS(paste0(here("Previne","data","rds"),
                                      "/CNES_ESF_Horario2020.RDS"))
    #Alternative CNES_ESF_Horario2020_full.RDS
  
    CNES_ESF_HorarioExt_short <- CNES_ESF_HorarioExt %>%
      group_by(CO_MUNICIPIO, Ano, Mes) %>%
...
saveRDS(CNES_variables, (paste0(here("Previne","data","rds"), "/CNES_Complete_2020.RDS")))
